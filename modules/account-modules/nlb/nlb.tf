...
variable "securityGroupIds" {
  type        = list(string)
  default     = []
  description = "Optional list of security group IDs to attach to the NLB. Empty list means no SG attached (legacy behavior)."
}
...
output "securityGroupId" {
  value = length(var.securityGroupIds) > 0 ? var.securityGroupIds[0] : null
}