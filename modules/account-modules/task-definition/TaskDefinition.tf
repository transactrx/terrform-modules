...
variable "additionalTrustedServices" {
  type        = list(string)
  default     = []
}

locals {
  trusted_services = concat(["ecs-tasks.amazonaws.com"], var.additionalTrustedServices)
  unique_services = distinct(local.trusted_services)
 ...
 "Statement": [
   {
     "Action": "sts:AssumeRole",
     "Principal": {
       "Service": ${jsonencode(local.unique_services)}
     },
     "Effect": "Allow",
     "Sid": ""
   }
